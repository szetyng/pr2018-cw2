...
kpca = KernelPCA(n_components=num_componet, kernel="rbf", gamma=0.00001)
kpca.fit(train_data)
gallery_kpca = kpca.transform(gallery_data)
query_kpca = kpca.transform(query_data)


features_kpca = np.zeros((14096, num_componet))
for i in range(query_idx.shape[0]):
    features_kpca[query_idx[i]] = query_kpca[i]
for i in range(gallery_idx.shape[0]):
    features_kpca[gallery_idx[i]] = gallery_kpca[i]

# Get k nearest neighbours
start1 = time.time()
nn2_idx = knn_camspecific(k,features_kpca, labels, query_idx, gallery_idx, camId)
end1 = time.time()
print(end1 - start1, 's')
