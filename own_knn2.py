...
def knn_camspecific(k,features, labels, query_idx, gallery_idx, camId):
    ...
    for aa,query_id in enumerate(query_idx):
        ...
        for i in gallery_idx_f:
            gallery_data_f.append(features[i])
        gallery_data_f = np.array(gallery_data_f)

        distances = np.linalg.norm((query-gallery_data_f), axis=1)
        neighbourid_dist_pair = [(gallery_idx_f[e], distances[e]) for e in range(len(gallery_idx_f))]
...
start1 = time.time()
nn2_idx,distances = knn_camspecific(1,features, labels, query_idx, gallery_idx, camId)
end1 = time.time()
print(end1 - start1, 's')
